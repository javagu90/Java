...
import tms_incidencias.solicitud.TmsExcepcionIncidenciaFacadeRemote;
...
public class SesionVenta {
    ...
    private TmsExcepcionIncidenciaFacadeRemote tmsExcepcionIncidenciaFacade;
    ...
    public int proceso(long MENU_ID, String pIpAS, int pPortAS) {
        ...
        try {
            ...
            tmsExcepcionIncidenciaFacade = (TmsExcepcionIncidenciaFacadeRemote) context.lookup("tms_incidencias.solicitud.TmsExcepcionIncidenciaFacadeRemote");
        } catch (Exception ex) {
    ...
    private boolean cargaTerminal() {
        ...
        if (datosTerminal == null) {
            return false;
        }
        if (datosTerminal[0].length() > 3) {
            PrefijoTerminalId = datosTerminal[0].substring(0, 3);
        }
        if (datosTerminal[0].length() == 3) {
            PrefijoTerminalId = datosTerminal[0];
        }
        if (datosTerminal[0].length() == 2) {
            PrefijoTerminalId = datosTerminal[0] + "0";
        }
        if (datosTerminal[0].length() == 1) {
            PrefijoTerminalId = datosTerminal[0] + "00";
        }
    ...
    public String getAccionNombre(long accionId) {
        for (int i = 0; i < tmsAccionesTbl.size(); i++) {
            if (tmsAccionesTbl.get(i).getAccionId() == accionId) {
                return tmsAccionesTbl.get(i).getAccion();
            }
        }
    ...
    public long getAccionId(String accion) {
        for (int i = 0; i < tmsAccionesTbl.size(); i++) {
            if (tmsAccionesTbl.get(i).getAccion().equals(accion)) {
                return tmsAccionesTbl.get(i).getAccionId();
            }
        }
    ...
    public String getActividad(long actId) {
        for (int i = 0; i < tmsIncidenciasTbl.size(); i++) {
            if (tmsIncidenciasTbl.get(i).getTipoActividadAdicionalId() == actId) {
                return tmsIncidenciasTbl.get(i).getActividadClave();
            }
        }
        return "GUARDIA";
    }
    ...
    public List<TmsActAdicionalesTbl> getTmsIncidenciasTbl() {
        return this.tmsIncidenciasTbl;
    }

    public List<TmsServiciosTbl> getTmsServiciosTbl() {
        return this.tmsServiciosTbl;
    }

    public List<TmsEmpresasTbl> getTmsEmpresasTbl() {
        return this.tmsEmpresasTbl;
    }
    ...
    public Object[][] busqueda(String claveOperador, String Servicio, String claveIncidencia, String fecha1, String fecha2, String estado) {
        if (claveOperador.equals("")) {
            claveOperador = "%";
        }
        if (Servicio.equals("TODOS")) {
            Servicio = "";
        }
        if (claveIncidencia.equals("TODOS")) {
            claveIncidencia = "";
        }
        ...
        if (vectorIncOper == null) {
            return null;
        }
        IncOper = new ArrayList<vwIncOper>();
        Object[][] resultado = new Object[vectorIncOper.size()][6];
        Vector y;
        vwIncOper VwIncOper;
        for (int i = 0; i < vectorIncOper.size(); i++) {
            ...
            resultado[i][2] = formatoTS1.format(new Date(VwIncOper.getFechaInicial().getTime()));
            resultado[i][3] = (VwIncOper.getFechaFinal() == null ? "" : formatoTS1.format(new Date(VwIncOper.getFechaFinal().getTime())));
            resultado[i][4] = VwIncOper.getServicioNombre();
            resultado[i][5] = VwIncOper.getIncidenciaAutorizada();
        }

        return resultado;
    }

    public List<vwIncOper> getVwIncOper() {
        return this.IncOper;
    }
    ...
    public void setFilaIncidenciaSeleccionada(int fila) {
        this.filaIncidenciaSeleccionada = fila;
    }
    ...
    public List<TmsEstadoOperadoresV> getTmsEstadoOperadoresV() {
        return this.objTmsOperadoresV;
    }

    public int getFilaOperadorSeleccionado() {
        return this.filaOperadorSeleccionado;
    }

    public void setFilaOperadorSeleccionado(int fila) {
        this.filaOperadorSeleccionado = fila;
    }
    ...
    public long getOperIncId() {
        return this.operIncId;
    }
    ...
    public String getNombreOperador(String clave) {
        for (int i = 0; i < this.tmsOperadoresV.size(); i++) {
            if (this.tmsOperadoresV.get(i).getClaveOperador().equals(clave)) {
                return this.tmsOperadoresV.get(i).getOperadorNombreCompleto();
            }
        }
        return "S/N";
    }
    ...
    public String[][] xDatosAdicionales() {
    ...
    public boolean ingresarPreIngreso(long usuarioId) {
        ...
        if (getVwIncOper().get(getFilaIncidenciaSeleccionada()).getAplicaRecaudacion().equals("S")) {
            if (!PreIngresoRecaudacion(usuarioId, getVwIncOper().get(getFilaIncidenciaSeleccionada()).getIncidenciaId())) {
                return false;
            }
        }
        if (!autorizaIncidencia(getVwIncOper().get(getFilaIncidenciaSeleccionada()).getOperIncidenciaId(), usuarioId)) {
            return false;
        }
        return true;
    }
       ...
       public boolean ingresarPreIngresoEspecial(long usuarioId) {
        if (getEspecial().getAplicaRecaudacion().equals("S")) {
            if (!PreIngresoRecaudacionEspecial(usuarioId, getEspecial().getIncidenciaId())) {
                return false;
            }
        }
        if (!autorizaIncidencia(getEspecial().getOperIncidenciaId(), usuarioId/*,
                getEspecial().getOperadorId(), getEspecial().getClaveOperador(), getEspecial().getIncidenciaClave()*/)) {
            return false;
        }
        return true;
    }
    ...
    private boolean PreIngresoRecaudacion(long usuarioId, long tipoActAdicId) {
        ...
        if (xDatosAdicionales() != null) {
            for (int i = 0; i < xDatosAdicionales().length; i++) {
                if (xDatosAdicionales()[i][1].equals("ADICIONAL1")) {
                    tmsPagosActAdicionalesTbl.setAdicional1(xDatosAdicionales()[i][2]);
                }
                if (xDatosAdicionales()[i][1].equals("ADICIONAL2")) {
                    tmsPagosActAdicionalesTbl.setAdicional2(xDatosAdicionales()[i][2]);
                }
                if (xDatosAdicionales()[i][1].equals("ADICIONAL3")) {
                    tmsPagosActAdicionalesTbl.setAdicional3(xDatosAdicionales()[i][2]);
                }
                if (xDatosAdicionales()[i][1].equals("ADICIONAL4")) {
                    tmsPagosActAdicionalesTbl.setAdicional4(xDatosAdicionales()[i][2]);
                }
                if (xDatosAdicionales()[i][1].equals("ADICIONAL5")) {
                    tmsPagosActAdicionalesTbl.setAdicional5(xDatosAdicionales()[i][2]);
                }
                if (xDatosAdicionales()[i][1].equals("ADICIONAL6")) {
                    tmsPagosActAdicionalesTbl.setAdicional6(xDatosAdicionales()[i][2]);
                }
                if (xDatosAdicionales()[i][1].equals("ADICIONAL7")) {
                    tmsPagosActAdicionalesTbl.setAdicional7(xDatosAdicionales()[i][2]);
                }
                if (xDatosAdicionales()[i][1].equals("ADICIONAL8")) {
                    tmsPagosActAdicionalesTbl.setAdicional8(xDatosAdicionales()[i][2]);
                }
                if (xDatosAdicionales()[i][1].equals("ADICIONAL9")) {
                    tmsPagosActAdicionalesTbl.setAdicional9(xDatosAdicionales()[i][2]);
                }
                if (xDatosAdicionales()[i][1].equals("ADICIONAL10")) {
                    tmsPagosActAdicionalesTbl.setAdicional10(xDatosAdicionales()[i][2]);
                }
                if (xDatosAdicionales()[i][1].equals("ADICIONAL11")) {
                    tmsPagosActAdicionalesTbl.setAdicional11(xDatosAdicionales()[i][2]);
                }
                if (xDatosAdicionales()[i][1].equals("ADICIONAL12")) {
                    tmsPagosActAdicionalesTbl.setAdicional12(xDatosAdicionales()[i][2]);
                }
                if (xDatosAdicionales()[i][1].equals("ADICIONAL13")) {
                    tmsPagosActAdicionalesTbl.setAdicional13(xDatosAdicionales()[i][2]);
                }
                if (xDatosAdicionales()[i][1].equals("ADICIONAL14")) {
                    tmsPagosActAdicionalesTbl.setAdicional14(xDatosAdicionales()[i][2]);
                }
                if (xDatosAdicionales()[i][1].equals("ADICIONAL15")) {
                    tmsPagosActAdicionalesTbl.setAdicional15(xDatosAdicionales()[i][2]);
                }
                if (xDatosAdicionales()[i][1].equals("ADICIONAL16")) {
                    tmsPagosActAdicionalesTbl.setAdicional16(xDatosAdicionales()[i][2]);
                }
                if (xDatosAdicionales()[i][1].equals("ADICIONAL17")) {
                    tmsPagosActAdicionalesTbl.setAdicional17(xDatosAdicionales()[i][2]);
                }
                if (xDatosAdicionales()[i][1].equals("ADICIONAL18")) {
                    tmsPagosActAdicionalesTbl.setAdicional18(xDatosAdicionales()[i][2]);
                }
                if (xDatosAdicionales()[i][1].equals("ADICIONAL19")) {
                    tmsPagosActAdicionalesTbl.setAdicional19(xDatosAdicionales()[i][2]);
                }
                if (xDatosAdicionales()[i][1].equals("ADICIONAL20")) {
                    tmsPagosActAdicionalesTbl.setAdicional20(xDatosAdicionales()[i][2]);
                }
                if (xDatosAdicionales()[i][1].equals("ADICIONAL21")) {
                    tmsPagosActAdicionalesTbl.setAdicional21(xDatosAdicionales()[i][2]);
                }
                if (xDatosAdicionales()[i][1].equals("ADICIONAL22")) {
                    tmsPagosActAdicionalesTbl.setAdicional22(xDatosAdicionales()[i][2]);
                }
                if (xDatosAdicionales()[i][1].equals("ADICIONAL23")) {
                    tmsPagosActAdicionalesTbl.setAdicional23(xDatosAdicionales()[i][2]);
                }
                if (xDatosAdicionales()[i][1].equals("ADICIONAL24")) {
                    tmsPagosActAdicionalesTbl.setAdicional24(xDatosAdicionales()[i][2]);
                }
                if (xDatosAdicionales()[i][1].equals("ADICIONAL25")) {
                    tmsPagosActAdicionalesTbl.setAdicional25(xDatosAdicionales()[i][2]);
                }
                if (xDatosAdicionales()[i][1].equals("ADICIONAL26")) {
                    tmsPagosActAdicionalesTbl.setAdicional26(xDatosAdicionales()[i][2]);
                }
                if (xDatosAdicionales()[i][1].equals("ADICIONAL27")) {
                    tmsPagosActAdicionalesTbl.setAdicional27(xDatosAdicionales()[i][2]);
                }
                if (xDatosAdicionales()[i][1].equals("ADICIONAL28")) {
                    tmsPagosActAdicionalesTbl.setAdicional28(xDatosAdicionales()[i][2]);
                }
                if (xDatosAdicionales()[i][1].equals("ADICIONAL29")) {
                    tmsPagosActAdicionalesTbl.setAdicional29(xDatosAdicionales()[i][2]);
                }
                if (xDatosAdicionales()[i][1].equals("ADICIONAL30")) {
                    tmsPagosActAdicionalesTbl.setAdicional30(xDatosAdicionales()[i][2]);
                }
            }
        }
        try {
    ...
    public String getLigaCentral() {
        return this.dblCentral;
    }
    ...
    public void setIpAS(String valor) {
        this.ipAS = valor;
    }

    public void setPortAS(int valor) {
        this.portAS = valor;
    }
    ...
    /**
     * Obtiene un vector con las excepciones de incidencias
     * registradas
     * @return
     */
    public List<ExcepcionIncidencia> getAllExcepcionesIncidencias() throws ControlException {
         List<ExcepcionIncidencia> resultado = null;
        try{
        Vector r = tmsExcepcionIncidenciaFacade.getAllExcepcionesIncidencias();
        /*Vector r=new Vector();
        String[] incidencias1={"Vacaciones","Baja","Alta","Permiso","Ir al baño","Necesidades"};
        String[] incidencias2={"Baja","Vacaciones","Permiso","Ir al baño","Baja-ata","ADO"};
        for(int j=0;j<5;j++){
        Vector z=new Vector();
        z.add(new Short(""+j));
        z.add(incidencias1[j]);
        z.add(incidencias2[j]);
        z.add("SAMUDIO PEREZ EREZ");
        z.add(new Date());
        r.add(z);
        }*/
        ...
        if (r != null && r.size() > 0) {
            resultado = new ArrayList<ExcepcionIncidencia>();
            for (int i = 0; i < r.size(); i++) {
                Vector y = (Vector) r.get(i);
                ExcepcionIncidencia exc = new ExcepcionIncidencia();
                //Date f=(Date)y.get(4);
                exc.setFechaCreacion((String) y.get(4));
                exc.setId(((BigDecimal) y.get(0)).shortValue());
                exc.setIncidencia1((String) y.get(1));
                exc.setIncidencia2((String) y.get(2));
                exc.setUsuario((String) y.get(3));
                resultado.add(exc);
            }
        }
        }catch(Exception e){
            throw new ControlException("Ocurrio un error al buscar las excepciones de incidencia", e);
        }
        return resultado;
    }

    /**
     * Obtiene lista de actividades adicionales,
     * para obtener el id y la descripcion de la incidencia
     * @return  List<ActividadAdicional>
     */
    public List<ActividadAdicional> getActividadesAdicionalesIncidencias() {
        Vector r = tmsExcepcionIncidenciaFacade.getActAdicionales();
        /*Vector r=new Vector();
        String[] incidencias1={"Vacaciones","Baja","Alta","Permiso","Ir al baño","Necesidades"};
        for(int j=0;j<5;j++){
        Vector z=new Vector();
        ...
        List<ActividadAdicional> resultado = null;
        if (r != null && r.size() > 0) {
            resultado = new ArrayList<ActividadAdicional>();
            for (int i = 0; i < r.size(); i++) {
                Vector y = (Vector) r.get(i);
                ActividadAdicional act = new ActividadAdicional();
                act.setClaveIncidencia((String) y.get(1));
                act.setId(((BigDecimal) y.get(0)).longValue());
                act.setIncidencia((String) y.get(2));
                resultado.add(act);
            }
        }
        return resultado;
    ...
    /**
     * Se invoca el bean para guardar la excepcion
     * de incidencia en la base de datos
     * @param excepcionIncidencia
     * @throws ControlException
     * @throws ExisteExcepcionIncidenciaException
     */
    public void guardarExcepcionIncidencia(TmsExcepcionIncidenciaTbl excepcionIncidencia) throws ControlException, ExisteExcepcionIncidenciaException {
        try {
            tmsExcepcionIncidenciaFacade.guardarExcepcionIncidencia(excepcionIncidencia);
        } catch (Exception e) {
            System.err.println(e);
            if (e.getMessage() != null && e.getMessage().contains("TCENTRAL.IND_UNIQ_EXCEP_INC")) {
                throw new ExisteExcepcionIncidenciaException("Ya existe la Excepcion de Incidencia en el sistema");
            } else {
                throw new ControlException("Ocurrio un error al guardar excepcion de incidencia", e);
            }
        }
    ...
    /**
     * Invoca al bean para eliminar una excepcion de incidencia
     * en la base de datos
     * @param idExcepcionIncidencia
     * @throws ControlException
     */
    public void eliminarExcepcionIncidencia(Short idExcepcionIncidencia) throws ControlException {
        try {
            tmsExcepcionIncidenciaFacade.eliminarExcepcionIncidencia(idExcepcionIncidencia);
        } catch (Exception e) {
            System.err.println(e);
            throw new ControlException("Ocurrio un error al eliminar excepcion de incidencia", e);
        }
    }

    /**
     * Valida si la incidencia que se quiere registrar
     * no esta en las excepciones
     * @param claveOperador
     * @param fecha1
     * @param fecha2
     * @param idIncidencia
     * @return
     */
    public boolean validarExcepcionIncidencia(String claveOperador, String fecha1, String fecha2,Long idIncidencia) {
        if (!abreSocketAS()) {
            return false;
        }
        return tmsExcepcionIncidenciaFacade.validaExcepciones(claveOperador, fecha1, fecha2, idIncidencia);
    }

    /**
     * Se creo este metodo para solo autorizar la incidencia
     * sin el proceso de pagos
     * @param usuarioId
     * @return
     */
    public boolean autorizarIncidencia(long usuarioId) {
        if (!autorizaIncidencia(getVwIncOper().get(getFilaIncidenciaSeleccionada()).getOperIncidenciaId(), usuarioId)) {
            return false;
        }
        return true;
    ...
    /**
     * Valida si un usuario tiene permisos para visualizar
     * un menu u opcion del sistema
     * @param idUsuario
     * @param numeroFuncion
     * @return
     */
    public boolean tienePermisosUsuario(Long idUsuario,String numeroFuncion){
        return tmsExcepcionIncidenciaFacade.tienePermisosUsuario(idUsuario, numeroFuncion);
    }
}
