 ...
 * @author  imunoz
 */
public class jDlgRefoliar extends javax.swing.JDialog {
    
    /**
     * Creates new form jDlgDatosIniciales
     */
    public jDlgRefoliar(boolean edicion,boolean esNuevaSesion, double fondoMaximo, SesionVenta pSesionVenta, Vector pRefoliarEmpresas, boolean arranque){
        this.setModal(true);
        this.sesionVenta = pSesionVenta;
        this.RefoliarEmpresas = pRefoliarEmpresas;
        this.setFondoMaximo(fondoMaximo);
        initComponents();
        jLblBarra.setText("<html><font color="+ColoresInterfaz.cHTML3+">&#171; &#187;</font> Ingresar Datos | <font color="+ColoresInterfaz.cHTML3+">F10</font> Aceptar Dotacion Nueva | <font color="+ColoresInterfaz.cHTML3+">ESC</font> Cancelar<html>");
        interfazColor();
        ...
        if(arranque){
            ...
            if(RefoliarEmpresas.size()==1){
                f1=Long.valueOf(sesionVenta.obtenerFolioActual(RefoliarEmpresas.get(0).toString()));
                f2=sesionVenta.obtenerFolioFinal(RefoliarEmpresas.get(0).toString());
                f3=(f2-f1)+1;
            }
            else{
                f3=0;
        ...
        if(f3==0){
            ...
            ReglasPrimeraVez=true;
        }
        else{
            System.out.println("NO PRIMERA VEZ");
            ReglasPrimeraVez=false;
        }
    ...
    private void initComponents() {
        ...
        jLblBarra.setText("<html><font color=\"+ColoresInterfaz.cHTML1+\">&#171; &#187;</font> Ingresar Datos | <font color=\"+ColoresInterfaz.cHTML1+\">F10</font> Aceptar Dotacion Nueva | <font color=\"+ColoresInterfaz.cHTML1+\">ESC</font> Cancelar<html>");
        ...
        jPnlFondoLayout.setHorizontalGroup(
            ...
            .add(jPnlFondoLayout.createSequentialGroup()
                ...
                .addContainerGap())
            .add(jLblBarra, org.jdesktop.layout.GroupLayout.DEFAULT_SIZE, 514, Short.MAX_VALUE)
        );
        jPnlFondoLayout.setVerticalGroup(
            ...
            .add(jPnlFondoLayout.createSequentialGroup()
                ...
                .add(jPnlFondoLayout.createParallelGroup(org.jdesktop.layout.GroupLayout.BASELINE)
                    .add(jLabel1)
                    .add(jFrmTxtFondo, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 14, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE))
                .addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
                .add(jScrPnFolios, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 92, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
                .addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
...
// TODO add your handling code here:
    ...
    private void jTblFoliosKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_jTblFoliosKeyPressed
        switch(evt.getKeyCode()){
            ...
            case KeyEvent.VK_ENTER:
                //jFrmTxtFondo.requestFocusInWindow();
                break;
            case KeyEvent.VK_F10:
                if(ReglasPrimeraVez){
                    ...
                    if(DialogoSiNo.getResultado()){
                        validarCargaFolio();
                        if(this.getAccesoVenta()==0){ this.iniciaAppVenta = 0; this.dispose(); return;}
                    }
                }
                else{
                    validarCargaFolio();
        ...
        if(jFrmTxtFondo.getText().length() != 0 || !editarFonInicial){
            ...
            if(FondoInicial <= getFondoMaximo()){
                ...
                for(int i=0; i<empresasCantidad; i++){
                    ...
                    if(!folioDato.equals("") && !folioDato.equals("null") && !folioDato.equals("0") && !folioDato.equals("-")){
                        System.out.println("folioDato1(validarDatosIngresados):"+folioDato);
                        foliosInicial[i] = Integer.parseInt(folioDato);
                        folioDato = String.valueOf(tablaModeloEmpresas.getValueAt(i,2));
                        if (!folioDato.equals("") && !folioDato.equals("null") && !folioDato.equals("0")){
                            System.out.println("folioDato2(validarDatosIngresados):"+folioDato);
        ...
        for(int i=0;i<empresasCantidad;i++){
            ...
            switch(r){
                ...
                case 10: 
                    ...
                    return false;
                case 20: 
                    DialogoAceptar.mostrarDialogo("Folios.", "Uno o mas boletos dentro del Rango de Folios ha sido vendido.", Color.RED);
                    return false;
                case 30: 
    ...
    // MODIFICADO ERROR
    public int mostrarFoliosSesion(){
        String[][] folios=sesionVenta.getFoliosSesion();
        String nbEmpresa;
        int i, j, cont=0;
        for(i=0;i<RefoliarEmpresas.size();i++){
            nbEmpresa=RefoliarEmpresas.get(0).toString();
            for(j=0; j<folios.length; j++){
                if(folios[j][0] != null){
                    if(folios[j][2].equals(nbEmpresa)){
                        tablaModeloEmpresas.setValueAt(folios[j][0],i,1);
                        tablaModeloEmpresas.setValueAt(folios[j][1],i,2);
                        cont++;
                    }
                }
            }
    ...
    private void validarCargaFolio() {
        //Cargar Datos Iniciales en Sesión
    ...
    public void inicializarEmpresasNombre(boolean difEmpMismoFolio) {
        ...
        //empresasNombre = sesionVenta.getUserCon().getEmpresas();
        empresasNombre = new String[RefoliarEmpresas.size()][2];
        int i;
        for(i=0; i<empresasNombre.length; i++){
            empresasNombre[i][0] = String.valueOf(sesionVenta.getEmpresaId(RefoliarEmpresas.get(i).toString()));
            empresasNombre[i][1] = RefoliarEmpresas.get(i).toString();
        }
        //System.out.println("difEmpMismoFolio:"+difEmpMismoFolio);
        if (!difEmpMismoFolio){
            empresasCantidad = RefoliarEmpresas.size();
            for(i=0;i<empresasCantidad;i++){
                    empresasDatosTabla[0] = empresasNombre[i][1];
            ...
            empresasCantidad = 1;
            for(i=0;i<empresasCantidad;i++){
                empresasDatosTabla[0] = empresasNombre[0][1];
                empresasDatosTabla[1] = Integer.parseInt("0");
                empresasDatosTabla[2] = Integer.parseInt("0");
                
                //Inicializar tabla de empresas para una nada mas
                empresasDatosTexto[0][0] = empresasNombre[i][0];
                empresasDatosTexto[0][1] = empresasNombre[i][1];
        ...
        //System.out.println("FOLIA INDICE EMPRESA "+this.indiceEmpresa);
    }
    ...
    private void inicializarTablaEmpresas(){
        tablaModeloEmpresas = new javax.swing.table.DefaultTableModel(
            ...
            public boolean isCellEditable(int rowIndex, int columnIndex) {
                ...
                if(!ReglasPrimeraVez && columnIndex==2) return false;
                return true;
            }
        };
    }
    ...
    private int iniciaAppVenta=0;
    private Vector RefoliarEmpresas;
    //private int indiceEmpresa=-1;

    private JDlgAceptar DialogoAceptar = new JDlgAceptar();
    private boolean ReglasPrimeraVez=false;
