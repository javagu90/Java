...
// TODO add your handling code here:
    ...
    private void validarSupervisor(){
        ...
        if(numero.length() == 0){
            DialogoAceptar = new JDlgAceptar("Validar Clave", "<html>El numero de usuario no puede estar vacio.<br>Por favor intenta de nuevo.</html>", Color.RED);
        }else{
            if(clave.length() == 0){
                DialogoAceptar = new JDlgAceptar("Validar Clave", "<html>La Contraseña de usuario no puede estar vacia.\nPor favor intenta de nuevo.</html>",Color.RED);
            }else{
                //System.out.println(" PERMISO PARA "+numeroFuncion+" "+numero+" "+clave);
                if(!sesionVenta.validarClaveSupervisorConFuncion(numeroFuncion, numero, clave)){
                    respuesta = false;
                    DialogoAceptar = new JDlgAceptar("¡Acceso denegado!", "<html>La clave proporcionada no es correcta,<br>o no tiene permiso para realizar esta transaccion.</html>", Color.RED);
