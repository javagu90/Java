...
import eausuariofuncion.AppPrincipal;
...
public class jlaypnl_login extends JLayeredPane {
    ...
    public jlaypnl_login(login_main p) {
        ...
        iniciarConexion();   
        principal.setTitle("TMS Venta v2.9 "+ventaLoginCnx.getNombreTerminal()+" - Inicio de Sesión");
        principal.setTitle("TMS Venta v2.10 "+ventaLoginCnx.getNombreTerminal()+" - Inicio de Sesión");
     //   System.out.println("Revision TMS Venta 23 de Octubre de 2014");

        System.out.println("Revision TMS Venta 03 de Abril de 2015");
...
//        iniciarConexion();
        if (validarUsuario(usuarioNumero,pwdUsuario)){
                ...
                if(sesion) 
                {
                    System.out.println(" Validar Perfiles...");
    ...
    private void jtxt_contraseña_focusGained(FocusEvent e) {
        if((new String(jtxt_contraseña.getPassword())).equals(""))
        ;
        else
         jtxt_contraseña.selectAll();
    ...
    {
      boolean valor = false;  
      System.out.println("id_usuario Testing...:"+id_usuario);
