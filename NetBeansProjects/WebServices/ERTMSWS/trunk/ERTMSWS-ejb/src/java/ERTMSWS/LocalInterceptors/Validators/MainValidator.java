 ...
 *
 * Root class para todos los Interceptors de validación, definiendo métodos comunes para todos ellos
 *
...
public abstract class MainValidator {
    ...
    public MainValidator() {
        ...
        errorList.add(new ErrorMessage(NEGATIVO,"Valor numérico negativo"));
    ...
    public boolean isValidClaveCorrida(String value)throws Exception{
        //añadir código para verificar si es una clavecorrida válida
        if (value != null){
    ...
    public boolean isValidOrigen(String value)throws Exception{
        //Añadir código para recuperar de BD los Origenes-Destinos válidos
        if (value != null){
            if (this.isValidLength(value,30))
                return true;
            else
    ...
    public boolean isValidSesionId(int  ID)throws Exception{
        ...
        //Añadir código para verificar si el ID provisto es válido según políticas del negocio
        if (ID >= 0)
            return true;
        else{
    ...
    public boolean isValidAsientos(Asientos[] asientos) throws Exception{
        if (asientos != null)
        //Añadir código de validación de asientos
            return true;
        else
    ...
    public String getErrorMessage(int error,String variable){
        for (ErrorMessage em : errorList){
            if (error == em.geterrorCode())
                return "(Código de Error)"+error+": (Campo)"+variable+" -- "+em.getmensaje();
        }
        return "-1 "+variable+": Código de Error no encontrado";
    }
    ...
    public static void responseErrorClass(ExchangeResp exResp, Exception except){
        /* Aprovechando características de polimorfismo, se crea método general para la
         * generación de las clases de respuesta cuando es generada una excepción
         */
    ...
    public void printParameters(Object ob) throws Exception{
         ...
         **/
            ...
            System.out.println("*************** Ver Exception.....");
            ex.printStackTrace();
        }
    }
}
