...
public class ValeBean extends AbstractBean implements Serializable {
	...
	public void showCreateDialogListener() {
		...
		vale = new Vale();
		vale.setFolio(Utils.getFolio(0, 0,ApplicationBean.MODULE_PREFIX_VALE, ""));
	...
	public String displaySelectedView(ActionEvent event) throws IOException {		
		DefaultMenuItem item = (DefaultMenuItem) ((MenuActionEvent) event).getMenuItem();
		String paramValue = item.getParams().get(Utils.PARAM_MENU_VALES).get(0);
		if (paramValue.endsWith("List.xhtml")) {
			...
			return paramValue + "?faces-redirect=true";
		}
		return null;
	}
}
