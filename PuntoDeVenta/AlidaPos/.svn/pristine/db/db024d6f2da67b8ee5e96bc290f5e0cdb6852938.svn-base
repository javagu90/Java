...
public class ProductoFacade implements Serializable {
    ...
    public List<Producto> FindProductoLikeNameTienda(String name, Tienda tienda) throws AlidaPosException {
    	try {
    		productoDao.beginTransaction();
    		List<Producto> result = productoDao.findLikeNameTienda(name, tienda);    		
    		productoDao.closeTransaction();
        	return result;
    	} catch (Exception ex) {
    		try {
