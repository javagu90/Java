...
@NamedQueries({
    ...
    @NamedQuery(name = "Producto.findLikeNombre", query = "SELECT p FROM Producto p WHERE UPPER(p.nombre) LIKE UPPER(:nombre) OR UPPER(p.clave) LIKE UPPER(:nombre) OR UPPER(p.codigo) LIKE UPPER(:nombre)"),
    ...
    @NamedQuery(name = "Producto.findLikeNombreTienda", query = "SELECT p FROM Producto p WHERE (UPPER(p.nombre) LIKE UPPER(:nombre) OR UPPER(p.clave) LIKE UPPER(:nombre) OR UPPER(p.codigo) LIKE UPPER(:nombre)) "
                                                     		+ "AND p.idproducto in (SELECT d.producto.idproducto FROM Detalleinventario d WHERE d.inventario.idinventario = "
                                                     		+ "( SELECT i.idinventario FROM Inventario i WHERE i.tienda = :tienda)) ") })
public class Producto implements Serializable {
	...
	public static String FIND_LIKE_NOMBRE_TIENDA       = "Producto.findLikeNombreTienda";
	...
	public void setExentoDeIva(boolean exentoDeIva) {
		this.exentoDeIva = exentoDeIva;
	}
