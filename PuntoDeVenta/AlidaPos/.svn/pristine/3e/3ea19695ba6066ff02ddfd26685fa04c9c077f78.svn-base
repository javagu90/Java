...
public class GenericDao<T> {
	...
	public List getListResultNative(String nativeQuery, Map<String, Object> parameters) throws AlidaPosException {
		List result = null;
		try {
			...
			e.printStackTrace();
			throw new AlidaPosException("Error al ejecutar Consulta." + e.getMessage());
		}
		return result;
	}
	
	@SuppressWarnings("rawtypes")
	public List getListResultNative(String nativeQuery, Class classType, List<Object> parameters) throws AlidaPosException {
		List result = null;
		try {
			Query query = em.createNativeQuery(nativeQuery, classType);
			if (parameters != null && !parameters.isEmpty()) {
			...
			e.printStackTrace();
			throw new AlidaPosException("Error al ejecutar Consulta." + classType.toString() + "\n" + e.getMessage());
		}
		return result;
	}
	...
	private void populateNativeQueryParameters(Query query, List<Object> parameters) {
		for (int i = 0; i < parameters.size(); i++) {
			if (parameters.get(i) instanceof Date) {
				query.setParameter(i + 1, (Date) parameters.get(i), TemporalType.DATE);
			} else {
