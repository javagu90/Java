	...
	*/
	...
	public void showEditDialogListener() {
		...
		try {
			...
			payment.addDetailViewList(vista.getDetallevistas(), inventarioTienda);
	...
	public void showCreateDialogListener() {
		...
		if (path.equals("")) {
			...
			showListDialog = false;
			vista = new Vista();
			vista.setFolio(Utils.getFolio(0, 0, ApplicationBean.MODULE_PREFIX_VISTA, ""));
	...
	public void showViewDialogListener() {
		...
		try {
			inventarioTienda = getInventarioFacade().findInventarioByTienda(getLoginBean().getUser().getTienda());
			payment.addDetailViewList(vistaSelected.getDetallevistas(), inventarioTienda);
		} catch (AlidaPosException ex) {			
			keepDialogOpen();
            displayErrorMessageToUser("Error: " + ex.getMessage());
		}
	...
	public void valueChangeVistaList() {	
		try {
			vista = null;
			vistaSelected = null;
			System.out.println("status: " + vistaStatus);
        ...
        if (Float.compare(diff, 0.0f) != 0) {
        	try {
	            ...
	            if (Float.compare(oldValue, newValue) < 0) {
	            	...
	            	payment.addProductDetail(editView, prod, null, diff);	            	
	            } else {
	            	System.out.println("Restar Diferencia : " + diff);
	            	payment.removeProductDetail(editView, prod, null, diff);
	            }
	            getVistaFacade().updateVista(vista);
			...
			if (producto != null && producto.getIdproducto() != 0) {
				...
				if (editMode) {
					try {
						...
						payment.removeProductDetail(detview, producto, null, 1);
						deleteZeroQuantity();
						getVistaFacade().updateVista(vista);
					...
					try {
						...
						payment.addProductDetail(detview, producto, null, 1);
