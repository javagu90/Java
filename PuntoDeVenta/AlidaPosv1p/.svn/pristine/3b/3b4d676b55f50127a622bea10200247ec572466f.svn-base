...
public class PaymentHelper implements Serializable {
	...
	public void addDetailViewList(List<Detallevista> viewList) throws AlidaPosException {
		Detallevista view = null;
		for (int i = 0; i < viewList.size(); i++) {
			view = viewList.get(i);
			getProductDetailInformation(view, view.getProducto(), view.getCantidad());
		}
	}
	
	public void addDetailSaleList(List<Detalleventa> saleList) throws AlidaPosException {
		Detalleventa sale = null;
		for (int i = 0; i < saleList.size(); i++) {
			sale = saleList.get(i);
			getProductDetailInformation(sale, sale.getProducto(), sale.getCantidad());
		}
	}	
	
	public void addDetailApartadoList(List<Detalleapartado> apartadoList) throws AlidaPosException {
		Detalleapartado apartado = null;
		for (int i = 0; i< apartadoList.size(); i++) {
			apartado = apartadoList.get(i);
			getProductDetailInformation(apartado, apartado.getProducto(), apartado.getCantidad());
		}
	}
	
	public void getProductDetailInformation(Object object, Producto product, float quantity) throws AlidaPosException {
		...
		if (object instanceof Detalleapartado) {
			...
			discountHelper = applyPromotion(product, amountValue, detailInventory);
	...
	public boolean removeProductDetail(Object object, Producto product, float quantity) throws AlidaPosException {
		float priceValue              = 0.0f;
		float amountValue             = 0.0f;
		float taxValue                = 0.0f;
	...
	public boolean addProductDetail(Object object, Producto product, float quantity) throws AlidaPosException {		
		float priceValue  = 0.0f;
		float amountValue = 0.0f;
		float taxValue    = 0.0f;
		float discount    = 0.0f;
	...
	public DiscountHelper applyPromotion(Producto product, float price, Detalleinventario detailInventory) throws AlidaPosException {
		...
		if	(detailInventory == null) {
			detailInventory = getDetailStockFacade().findDetalleInventarioByProduct(product.getIdproducto());
		}
		...
		if (detailInventory.getPromocional()) {
			...
			try {
				promotion = getPromotionFacade().findPromocionByProduct(product.getIdproducto());
				if (promotion != null) {
